...
class MapPanel extends Panel
{
    ...
    public void addToPanel(Path[][] r) { paths = r;
        setLayout(new GridLayout(paths.length, paths[0].length, 0, 0));
        for (int row = 0; row < paths.length; row++) { for (int col = 0; col < paths[0].length; col++) { add("", paths[row][col]); }
        }
    }
    ...
    public void update(Graphics g) {
        ...
        if (backBuffer == null
            ...
            || backBuffer.getHeight(null) != h) { backBuffer = createImage(w, h);
            if (backBuffer != null) {
